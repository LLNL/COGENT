function smooth_eqdsk(input_file, s)
  ...
  figure(2);
  h2 = contour(r,z,psi_lower_SN,80);
  cmin = min(psi(:));
  cmax = max(psi(:));
  caxis([cmin cmax]);
  colorbar
  title('Lower-SN flux function')
  axis image;

  figure(3);
  h3 = contour(r,z,psi_smoothed_norm,80);
  hold on
  contour(r,z,psi_smoothed_norm,[1.15 1.1 1.05 1.0 0.9],'-k','LineWidth',1)
  hold off
  ...
  hold on
  contour(r,z,psi_smoothed_norm,[1.1 1.0 0.9],'-k','LineWidth',1)
  hold off
  cmin=-0.1;
  cmax=0.1;
  caxis([cmin cmax]);
