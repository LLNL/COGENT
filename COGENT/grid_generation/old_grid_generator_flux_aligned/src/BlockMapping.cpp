...
BlockMapping::getMagneticFluxFromDCT( const RealVect& a_physical_coordinate ) const
{
   ...
   */
   ...
   if (m_dealigned_grid) {
      ...
      double linFlux = m_trans_rad * (fabs(x_new) - fabs(y_new));
...
BlockMapping::getMagFieldData( const RealVect& a_physical_coordinate ) const
{
   ...
   if (m_dealigned_grid) {
      ...
      double linFlux_x = m_trans_rad * (m_lin_1[0] * signum(x_new) - m_lin_2[0] * signum(y_new));
      double linFlux_y = m_trans_rad * (m_lin_1[1] * signum(x_new) - m_lin_2[1] * signum(y_new));
...
BlockMapping::getRadialDir(const RealVect& a_X, const double a_length, const RealVect& a_cut_dir) const
{
   ...
   */
   ...
   double blend_coeff_dist = exp(-pow(a_length/trans_length,4));
   double blend_coeff = blend_coeff_angle * blend_coeff_dist;
  
   if (m_dealigned_grid) {
    trans_length = 0.6; 
    blend_coeff = exp(-pow(vectMag(XtoXpt)/trans_length,4));
   }
