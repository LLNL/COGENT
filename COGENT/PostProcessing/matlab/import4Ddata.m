function [DataStr] = import4Ddata(dataPath,species)
  if nargin<2
      species = 1;
  end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
%%%   import 4D data from COGENT output folder
%%%
%%%   input parameters:
...
if(species==1) % wont work for old file output name format (2012 ish)
    plisting    = dir([dataPath,'plt.1.*.4d.hdf5']);
    plistingMap = dir([dataPath,'plt.1.*.4d.map.hdf5']);
else
    plisting    = dir([dataPath,'*.',num2str(species),'*.4d.hdf5']);
    plistingMap = dir([dataPath,'*.',num2str(species),'*.4d.map.hdf5']);
end
...
%
nt = length(plisting);
data0out = zeros(nx,nx2,nx3,nx4,nt);
for it=1:nt
    ...
    for b=1:numBlocks
        ...
        DataStr.block(b).Fce = zeros(nxb+1,nx2b+1,nx3b+1,nx4b+1,nt);
    end
    ...
    for b=1:numBlocks
        ...
        k0 = DataStr.block(b).k0;
        k1 = DataStr.block(b).k1;
        l0 = DataStr.block(b).l0;
        l1 = DataStr.block(b).l1;
        nxb = i1-i0;
        nx2b = j1-j0;
        nx3b = k1-k0;
        nx4b = l1-l0;
        ...
        data0ce(1:end-1,1:end-1,1:end-1,1:end-1) = data0cc(i0:i1-1,j0:j1-1,k0:k1-1,l0:l1-1);
...
DataStr.Fcc = data0out;  % data values at cell-center
