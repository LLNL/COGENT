...
void MomentOp::computeIntegrand( LevelData<FArrayBox>& a_integrand,
                                 ...
                                 const Kernel<FArrayBox>&         a_kernel ) const
{
...
// modifed computeIntegrand to work for arbitrary LevelData<FluxBox>
inline
void MomentOp::computeIntegrand( LevelData<FluxBox>&       a_integrand,
                                 const KineticSpecies&       a_kinetic_species,
                                 const LevelData<FluxBox>& a_function,
...
inline
void MomentOp::partialIntegralMu( CP1::LevelData<CP1::FluxBox>&  a_result,
                                  const LevelData<FluxBox>&      a_integrand,
                                  const ProblemDomain&           a_domain,
                                  const SliceSpec&               a_slice_mu) const
{
   if (a_result.isDefined()) {
   ...
   LevelData<FluxBox> degenerate_integrand(degenerate_grids, a_integrand.nComp(), degenerate_ghosts);
   ...
   sliceLevelFluxLocalOnly( a_result, degenerate_integrand, a_slice_mu );
}
...
inline
void MomentOp::partialIntegralVp( CFG::LevelData<CFG::FluxBox>&        a_result,
                                  const CP1::LevelData<CP1::FluxBox>&  a_integrand,
                                  const CP1::ProblemDomain&            a_domain,
                                  const CP1::SliceSpec&                a_slice_vp)       const
{
   if (a_result.isDefined()) {
   ...
   CP1::LevelData<CP1::FluxBox> degenerate_integrand(degenerate_grids, a_integrand.nComp(), degenerate_ghosts);
   ...
   CP1::FaceSumOp op_vp( VPARALLEL_DIR );
   // op_vp.setScale(1.);
   a_integrand.copyTo( a_integrand.interval(),
                       degenerate_integrand,
                       degenerate_integrand.interval(),
   ...
   sliceLevelFluxLocalOnly( a_result, degenerate_integrand, a_slice_vp );
}
...
inline
void scaleResult( CFG::LevelData<CFG::FluxBox>& a_result,
                  const Real&                     a_factor,
                  const PhaseGeom&                a_coords )
{
   const CFG::DisjointBoxLayout& boxes = a_result.getBoxes();
   ...
   for (dit.begin(); dit.ok(); ++dit)
   {
      for (int dir=0; dir<CFG_DIM; ++dir) {
         a_result[dit][dir].mult( a_factor );
      }
   }
}
...
// modified compute to take moments of arbitrary LevelData<FluxBox>
void MomentOp::compute( CFG::LevelData<CFG::FluxBox>&  a_result,
                        const KineticSpecies&          a_kinetic_species,
                        const LevelData<FluxBox>&      a_function,
                        const Kernel<FluxBox>&         a_kernel ) const
{
   ...
   CP1::LevelData<CP1::FluxBox> partial_integrand;
   partialIntegralMu( partial_integrand, integrand, domain, slice_mu );

   CP1::ProblemDomain partial_domain( sliceDomain( domain, slice_mu ) );

   CFG::LevelData<CFG::FluxBox> moment;
   partialIntegralVp( moment, partial_integrand, partial_domain, slice_vp );
