...
int OperatorData::operToCode(std::string ops)
{
    if(ops=="sin"||ops=="SIN"||ops=="Sin"){
        ...
        return ParsingSpace::HEAVISIDE_;
    }
    else if(ops=="rand"||ops=="RAND"||ops=="Rand"){
        return ParsingSpace::RANDOM_;
    }
    else if(ops=="atwotan"||ops=="ATWOTAN"||ops=="Atwotan"){
        return ParsingSpace::YATANX_;
    }
...
std::string OperatorData::codeToStr()
{
    ...
    switch(m_idenCode){
        ...
        case ParsingSpace::HEAVISIDE_  :tempStr="h"; break;
        case ParsingSpace::RANDOM_     :tempStr="rand"; break;
        case ParsingSpace::YATANX_     :tempStr="atwotan"; break;
...
int OperatorData::setOperatorData(std::string ops)
{
    ...
    switch(m_idenCode){
        ...
        case ParsingSpace::TANH_:
            ...
            m_priority=ParsingSpace::PR5_;//30;
            break;
        case ParsingSpace::HEAVISIDE_:
            m_preInPost=ParsingSpace::PREFIX_;
            m_priority=ParsingSpace::PR5_;//30;
            break;
        case ParsingSpace::RANDOM_:
            m_preInPost=ParsingSpace::PREFIX_;
            m_priority=ParsingSpace::PR5_;//30;
            break;
        case ParsingSpace::YATANX_:
...
int ParsingCore::preProcess()
{
    ...
    std::string preFixList[]={
        "arcsin","ARCSIN","Arcsin","asin","ASIN","Asin",
...
//debug
#ifdef DEBUG_PARSER
        std::cout<<"found " <<postFixList[j]<<":"<<"manipStr="<<manipStr<<"  : preprocess 2.1"<<std::endl;
#endif
//debug
            }
            else{
...
//debug
#ifdef DEBUG_PARSER
        std::cout<<"found " <<postFixList[j]<<":"<<"manipStr="<<manipStr<<"  : preprocess 2.2"<<std::endl;
#endif
//debug
            }
        }
...
//debug
#ifdef DEBUG_PARSER
        std::cout<<"preFixList["<<j<<"]="<<preFixList[j]<<"  : preprocess 6"<<std::endl;
#endif
//debug
...
//debug
#ifdef DEBUG_PARSER
        std::cout<<"manipStr="<<manipStr<<"  : preprocess 6.1"<<std::endl;
#endif
//debug
            }
        }
...
//debug
#ifdef DEBUG_PARSER
        std::cout<<"manipStr="<<manipStr<<"  : preprocess 20"<<std::endl;
#endif
//debug
                    if(i==0){
                        manipStr[i]='E';
...
//debug
#ifdef DEBUG_PARSER
        std::cout<<"manipStr="<<manipStr<<"  : preprocess 21"<<std::endl;
#endif
//debug
                    }
                    else if( isdigit(manipStr[i-1])||manipStr[i-1]=='.' ){
...
//debug
#ifdef DEBUG_PARSER
        std::cout<<"manipStr="<<manipStr<<"  : preprocess 22"<<std::endl;
#endif
//debug
                    }
                    else{
...
//debug
#ifdef DEBUG_PARSER
        std::cout<<"manipStr="<<manipStr<<"  : preprocess 23"<<std::endl;
#endif
//debug
                    }
...
//debug
#ifdef DEBUG_PARSER
        std::cout<<"manipStr="<<manipStr<<"  : preprocess 100"<<std::endl;
#endif
//debug
    ...
    srand(time(NULL)); //Seed for userFuncRandom(). This function should be called once.
            ...
            if( serviceOperData.getPreInPost()==ParsingSpace::MACONST_ ){
                switch(serviceOperData.getIdenCode() ){
                    ...
                    case ParsingSpace::ANS__:
                        parserStr.resize(0);//Empty();
                        std::stringstream ss;
                        ss.precision(PARSING_STR_PRECISION);
                        ss <<std::scientific<< exAnswer;
                        parserStr=ss.str();
    ...
    if(isRad==true){
        switch(op.getIdenCode() ){
            ...
            case ParsingSpace::TANH_:
                ...
                break;
            case ParsingSpace::HEAVISIDE_:
                temp=userFuncHeaviside(operand1);
                break;
            case ParsingSpace::RANDOM_:
                temp=userFuncRandom(operand1);
                break;
            case ParsingSpace::PLUS_:
                temp=operand1;
                break;
...
double ParsingCore::calculate(double operand1, OperatorData op, double operand2)
{
    ...
    switch(op.getIdenCode() ){
        ...
        case ParsingSpace::DIVIDE_:
            ...
            break;
        case ParsingSpace::YATANX_:
            //temp=atan2(operand1,operand2);
            temp=userFuncAtan2(operand1,operand2); //0 to 2pi
            break;
        case ParsingSpace::POWER_:
            temp=pow(operand1,operand2);
            break;
        case ParsingSpace::NSQRT_:
            temp=pow(operand1,1.0/operand2);
            break;
        case ParsingSpace::EI_:
...
double ParsingCore::userFuncHeaviside( double operand1)
{
    double result=0.0;
    if (operand1>=0.0){
        result = 1.0;
...
double ParsingCore::userFuncRandom( double operand1 )
{
    //srand(time(null)) is called in the preProcess() method.
    return ( (double) (rand()) / (double) (RAND_MAX) )* operand1;
}

double ParsingCore::userFuncAtan2( double operand1 , double operand2)
{
    //atan2(y,x) but result from 0 to 2pi
    double result=0.0;
    result = atan2(operand1, operand2);
    if (result < 0.0 ){
        if (isRad==true){
            result += 2.0*M_PI;
        }
