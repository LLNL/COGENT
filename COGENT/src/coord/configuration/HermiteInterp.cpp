...
extern "C" {
  void rgbi3p_(const int& md, const int& nxd, const int& nyd,
...
HermiteInterp::HermiteInterp( const ParmParse& a_parm_parse,
                              const FArrayBox& a_coords,
                              const FArrayBox& a_data )
   : Interp(a_coords, a_data)
{
  if (SpaceDim != 2) {
    MayDay::Error("HermiteInterp is only two-dimensional");
  }
  ...
  for (int component=0; component<num_components; ++component) {
    ...
    rgbi3p_(1, dim(0), dim(1), m_x, m_y, m_data.dataPtr(component), 1,
            &m_x[0], &m_y[1], &dummy, ier, 0, m_wk[component]);
    if ( ier != 0 ) {
      cout << "ier = " << ier << endl;
      MayDay::Error("HermiteInterp(): RGBI3P returned a nonzero value");
    }
  }
}
...
double HermiteInterp::interpolate(const int       a_component,
                                  const int       a_mode,
                                  const RealVect& a_xi) const
{
  ...
  rgbi3p_(2, dim(0), dim(1), m_x, m_y, m_data.dataPtr(a_component), 1,
        &(a_xi[0]), &(a_xi[1]), &val, ier, a_mode, m_wk[a_component]);

  if ( ier != 0 ) {
    cout << "ier = " << ier << endl;
    MayDay::Error("HermiteInterp::interpolate(): RGBI3P returned a nonzero value");
  }
...
void HermiteInterp::interpolate(const int        a_src_component,
                                ...
                                FArrayBox&       a_dXdXi) const
{
  ...
  int ier;
  rgbi3p_(2, dim(0), dim(1), m_x, m_y, m_data.dataPtr(a_src_component), num_points,
