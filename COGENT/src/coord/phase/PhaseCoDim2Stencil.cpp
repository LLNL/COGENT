...
PhaseCoDim2Stencil::PhaseCoDim2Stencil( const PhaseCoDim2Stencil& a_bc_stencil_coupling )
...
void
PhaseCoDim2Stencil::define( const Box&             a_codim1_box,
                            const RealVect&        a_dx,
                            const int&             a_dir,
...
void
PhaseCoDim2Stencil::getStencil( const IntVect&    a_iv,
                                ...
                                vector<double>&   a_weights ) const
{
   ...
   // If the block interface in the transverse direction is a
   // physical boundary, we fill the codim2 ghost with the
   // average of the values extrapolated from the codim1
   // values.  The following factor accomplishes that.
   ...
   if (m_order == 2) {
      ...
      MayDay::Error("PhaseCoDim2Stencil::getPhaseCoDim2Stencil(): Unimplemented order");
   }
}
