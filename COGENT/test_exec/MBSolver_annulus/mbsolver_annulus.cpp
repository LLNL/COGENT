                      ...
                      DisjointBoxLayout&        a_grids )
{
  ...
  prob_domain = ProblemDomain(bounding_box, a_is_periodic);
...
int main( int a_argc, char* a_argv[] )
{
   ...
   if (status==0) {
      ...
      ParmParse pp_oneblock("oneblock");

      std::vector<int> num_cells(SpaceDim);
      pp_oneblock.getarr("num_cells", num_cells, 0, SpaceDim);
      ...
      LogRectCoordSys* mb_coord_sys = new LogRectCoordSys(pp_oneblock, "oneblock", num_cells, is_periodic, decomp);
      ...
      getDisjointBoxLayout( *mb_coord_sys, decomposition, periodic, grids );
