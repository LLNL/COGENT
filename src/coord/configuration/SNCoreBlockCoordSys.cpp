...
#ifndef MODEL_GEOM
   if (m_spectral_field) tmp_hi_R = m_Rmax; //2.35
...
double SNCoreBlockCoordSys::getOuterRsep() const
{
...
void SNCoreBlockCoordSys::getN(FluxBox& a_N, const Box& a_box) const
{
   for (int dir=0; dir<SpaceDim; ++dir) {
...
void
SNCoreBlockCoordSys::getFaceCenteredRealCoords( const int a_dir, FArrayBox& a_x ) const
{
...
void
SNCoreBlockCoordSys::cellVol(FArrayBox&     a_vol,
                                 const FluxBox& a_N,
                                 const Box&     a_box ) const
{
...
RealVect
SNCoreBlockCoordSys::realCoordPointwise( const RealVect& a_xi ) const
{
   ...
   if (m_block_type == R_CORE || m_block_type == L_CORE) {
      ...
      if (m_block_type == L_CORE) length = m_pol_ref_length * (upperMappedCoordinate(POLOIDAL_DIR) - a_xi_tmp[POLOIDAL_DIR]) / mapped_block_width[POLOIDAL_DIR];
      ...
      if ( m_block_type == L_CORE ) x[0] = m_R0 - (x[0]-m_R0);
...
double SNCoreBlockCoordSys::getXlo(double psiVal ) const
{
...
double SNCoreBlockCoordSys::getZlo(double psiVal ) const
{
...
double SNCoreBlockCoordSys::getZhi(double psiVal ) const
{
   bool residual_tolerance_satisfied = false;
...
RealVect
SNCoreBlockCoordSys::arcLengthInverse(const double length) const
{
...
double SNCoreBlockCoordSys::psi(const RealVect& a_x) const
{
   double x = a_x[0] - m_R0;
   double z = a_x[1] - m_Z0;
...
double SNCoreBlockCoordSys::theta(const RealVect& a_x) const
{
   double x = a_x[0] - m_R0;
   double z = a_x[1] - m_Z0;
...
double SNCoreBlockCoordSys::psiAtR0(const double a_Z) const
{
   double z = a_Z - m_Z0;
   //CH_assert(z >= m_Zx);
...
double SNCoreBlockCoordSys::psiZ(const RealVect& a_x) const
{
   double z = a_x[1] - m_Z0;
   //CH_assert(z >= m_Zx);
...
double SNCoreBlockCoordSys::psiX(const RealVect& a_x) const
{
   double x = a_x[0] - m_R0;
   return -m_a * sin(m_a * x);
}

double SNCoreBlockCoordSys::thetaZ(const RealVect& a_x) const
{
   double z = a_x[1] - m_Z0;
   //CH_assert(z >= m_Zx);
...
double SNCoreBlockCoordSys::thetaX(const RealVect& a_x) const
{
   double x = a_x[0] - m_R0;
   return -1.0/(m_a*sin(m_a*x));
}

void SNCoreBlockCoordSys::normalizeBpScale() {
