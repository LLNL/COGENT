...
void
PhaseBlockCoordSys::getCellCenteredFluxCoords( FArrayBox& a_xi ) const
{
   ...
   for (BoxIterator bit( box ); bit.ok(); ++bit) {
      ...
      RealVect x;
      for (int dir(0); dir<PDIM; ++dir) {
         x[dir] = xi_array( iv, dir );
      }
      x[RADIAL_DIR] += m_configuration_coords.getInnerFluxLabel();

      VEL::RealVect xi_vel( vel_restrict( x ) );

      VEL::RealVect x_vel( m_velocity_coords.realCoord( xi_vel ) );
      for (int dir(CFG_DIM); dir<PDIM; ++dir) {
         x[dir] = x_vel[dir-CFG_DIM];
      }
