...
template < >
int linearSize<CH_XDIR::IntVect>( const CH_XDIR::IntVect& a_input )
{
   return(CH_XDIR::SpaceDim*sizeof(int));
}

template < >
void linearIn<CH_XDIR::IntVect>( CH_XDIR::IntVect& a_outputT, const void* const a_inBuf )
{
   int* intBuf = (int*)a_inBuf;
   for (int idir(0); idir<CH_XDIR::SpaceDim; idir++) {
      a_outputT[idir] = intBuf[idir];
   }
}

template < >
void linearOut<CH_XDIR::IntVect>( void* const a_outBuf, const CH_XDIR::IntVect& a_inputT )
{
   int* intBuf = (int*)a_outBuf;
   for (int idir(0); idir<CH_XDIR::SpaceDim; idir++) {
      intBuf[idir] = a_inputT[idir];
   }
}
...
template < >
int BaseFab<SparseCoupling>::size(const Box&      a_R,
