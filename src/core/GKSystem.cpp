     ...
     m_use_native_time_integrator( !a_use_external_TI )
{
   ...
   if (!m_use_native_time_integrator) {
      ...
      if (m_ti_class == "rk") {
         m_integrator = new TiRK<GKState, GKRHSData, GKOps>;
      }
      else if (m_ti_class == "ark") {
         m_integrator = new TiARK<GKState, GKRHSData, GKOps>;
      }
      else {
         MayDay::Error("Unrecognized input for m_ti_class.");
      }
      m_integrator->define( a_pp, m_ti_method, m_state_comp, BASE_DT );
...
void
GKSystem::createFields( CFG::FieldPtrVect& a_fields )
{
   // JAFH: Need to implement this for real depending on the usage

   /*
     Create the vector of time-evolving, non-species configuration space fields.
   */

   if ( m_verbosity && procID() == 0 ) {
      cout << "Adding configuration space field variables..." << endl;
   }

   int ghosts(0);
   for (int n(0); n<SpaceDim; ++n) {
      if (m_ghostVect[n] > ghosts) {
         ghosts = m_ghostVect[n];
      }
   }
   ...
   while ( more_fields ) {
      ...
      Real depth;
      if ( ppfield.contains("depth") ) {
         ppfield.get("depth", depth);
      }
      else {
         field_is_complete = false;
      }

      if ( field_is_complete ) {
         if ( procID() == 0 && m_verbosity ) {
            std::cout << "   " << name
                      << ": field with depth = " << depth
                      << std::endl;
         }
         
         // Create the species object
         CFG::Field* field( new CFG::Field( name, *m_mag_geom ) );
         field->data().define( m_mag_geom->grids(), depth, CFG::IntVect::Zero );

         // Add the new species to the solution vector
         CFG::FieldPtr field_ptr( field );
         a_fields.push_back( field_ptr );
...
GKSystem::createFluidSpecies( CFG::FluidSpeciesPtrVect& a_fluid_species )
{
   ...
   /*
     Create the vector of configuration space fluid species model (pointers),
     and when a fluid species is created, add it to the kineticfluid species
     vector number species.
   */
   ...
   // loop one more time for electrons.  Coding here assumes there is a single electron species.
   int i_off(1);
   bool reading_fluid_electrons(false);
   int i_electron(0);
   string electronname;

   while ( more_fluid_species || reading_fluid_electrons ) {

      bool species_is_complete(true);

      // look for data specifying another fluid species
      stringstream s;
      if ( !reading_fluid_electrons ){
         s << "fluid_species." << a_fluid_species.size() + i_off;
      }
      else{
         // Re-create string for parsing electron data on the (second) electron pass through this loop
         s << "fluid_species." << i_electron;
      }
      ...
      else { // electrons must have charge -1.0
         charge = -1.0;
      }

      std::string model;
      Real depth(CFG_DIM+2);
      if ( ppspecies.contains("model") ) {
         ppspecies.get("model", model);
         if (model == "isothermal") {
            depth = CFG_DIM+1;
         }
      }
      ...
      if ( species_is_complete ) {
         ...
         // Create the species object
         CFG::FluidSpecies* fluid_spec(
            new CFG::FluidSpecies( name, mass, charge, model, *m_mag_geom ) );
         fluid_spec->data().define( m_mag_geom->grids(),
                                    depth,
                                    CFG::IntVect::Zero );

         // Add the new species to the solution vector
         CFG::FluidSpeciesPtr species( fluid_spec );
         a_fluid_species.push_back( species );
      ...
      // If all species have been input and fluid electrons have been included,
      // then we need to go back and read them.
      if ( !more_fluid_species && m_using_electrons ) {
         reading_fluid_electrons = !reading_fluid_electrons;
      }
   }
                        ...
                        int&  a_step_number)
{
   ...
   m_integrator->advance( a_cur_time, m_state_comp );
                             ...
                             const double& cur_time )
{
   ...
   //Plot fields
   const CFG::FieldPtrVect& fields( m_state_phys.dataField() );
   for (int comp(0); comp<fields.size(); comp++) {
      const CFG::Field& field_comp( *(fields[comp]) );
      ...
      if (m_hdf_fields) {
         ...
         m_gk_ops->plotFields( filename, field_comp, cur_time );
      }
   }
                                    ...
                                    const double a_cur_dt )
{
   ...
   // JAFH: Need to do something similar here for m_fluid_species and m_fields
...
{
  m_gk_ops->postTimeStep( a_cur_step, a_cur_time, m_state_comp );
  m_gk_ops->divideJ( m_state_comp, m_state_phys );
  if (procID() == 0) {
    cout << "  ----\n";
...
void GKSystem::parseParameters( ParmParse&         a_ppgksys )
{
   ...
   // Should we make an hdf file for fluids?
   a_ppgksys.query("hdf_fields",m_hdf_fields);
