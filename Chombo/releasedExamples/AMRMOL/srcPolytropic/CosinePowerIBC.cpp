...
#include "CosinePowerIBCF_F.H"

#include "NamespaceHeader.H"

// Null constructor
CosinePowerIBC::CosinePowerIBC()
{
  m_isFortranCommonSet = false;
}

// Constructor which defines parameters used by Fortran routines
CosinePowerIBC::CosinePowerIBC(Real&           a_smallPressure,
                               const Real&     a_gamma,
...
void CosinePowerIBC::setFortranCommon(Real&           a_smallPressure,
                                      const Real&     a_gamma,
                                      const Real&     a_ambientDensity,
                                      const Real&     a_deltaDensity,
                                      const int&      a_pressure,
                                      ...
                                      const Real&     a_artvisc)
{
  ...
  FORT_COSINEPOWERSETF(CHF_REAL(a_smallPressure),
...
PhysIBC* CosinePowerIBC::new_physIBC()
{
  CosinePowerIBC* retval = new CosinePowerIBC();
...
void CosinePowerIBC::initialize(LevelData<FArrayBox>& a_U)
{
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    ...
    FORT_COSINEPOWERINITF(CHF_FRA(U),
                          CHF_CONST_REAL(m_dx),
                          CHF_BOX(uBox));
  }
}
...
// Set boundary fluxes
void CosinePowerIBC::primBC(FArrayBox&            a_WGdnv,
...
void CosinePowerIBC::setBdrySlopes(FArrayBox&       a_dW,
                                const FArrayBox& a_W,
                                const int&       a_dir,
                                const Real&      a_time)
{
...
void CosinePowerIBC::artViscBC(FArrayBox&       a_F,
                            const FArrayBox& a_U,
