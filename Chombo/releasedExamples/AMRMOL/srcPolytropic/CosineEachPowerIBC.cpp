...
#include "CosineEachPowerIBCF_F.H"

#include "NamespaceHeader.H"

// Null constructor
CosineEachPowerIBC::CosineEachPowerIBC()
{
  m_isFortranCommonSet = false;
}

// Constructor which defines parameters used by Fortran routines
CosineEachPowerIBC::CosineEachPowerIBC(Real&           a_smallPressure,
                               const Real&     a_gamma,
...
//     a_ambientDensity - Ambient density add to the density cosine power
//     a_deltaDensity   - Height of the density cosine power
//     a_pressure       - If 0, use isentropic pressure
//                        if 1, use the constant pressure of 1.0
//     a_center         - Center of the cosine power
//     a_radmax         - Cutoff radius of the cosine power
//     a_power          - Power of cosine (use 6)
//     a_velocity       - Initial velocity of the gas
//     a_artvisc        - Artificial viscosity coefficient
void CosineEachPowerIBC::setFortranCommon(Real&           a_smallPressure,
                                      const Real&     a_gamma,
                                      const Real&     a_ambientDensity,
                                      const Real&     a_deltaDensity,
                                      const int&      a_pressure,
                                      ...
                                      const Real&     a_artvisc)
{
  ...
  FORT_COSINEEACHPOWERSETF(CHF_REAL(a_smallPressure),
...
PhysIBC* CosineEachPowerIBC::new_physIBC()
{
  CosineEachPowerIBC* retval = new CosineEachPowerIBC();
...
void CosineEachPowerIBC::initialize(LevelData<FArrayBox>& a_U)
{
  ...
  for (dit.begin(); dit.ok(); ++dit)
  {
    ...
    FORT_COSINEEACHPOWERINITF(CHF_FRA(U),
                          CHF_CONST_REAL(m_dx),
                          CHF_BOX(uBox));
  }
}
...
// Set boundary fluxes
void CosineEachPowerIBC::primBC(FArrayBox&            a_WGdnv,
...
void CosineEachPowerIBC::setBdrySlopes(FArrayBox&       a_dW,
                                const FArrayBox& a_W,
                                const int&       a_dir,
                                const Real&      a_time)
{
...
void CosineEachPowerIBC::artViscBC(FArrayBox&       a_F,
                            const FArrayBox& a_U,
