...
#include "GaussianMapped2IBCF_F.H"
#include "LoHiSide.H"
...
// Null constructor
GaussianMappedMultiBlockIBC::GaussianMappedMultiBlockIBC()
  :
...
GaussianMappedMultiBlockIBC::GaussianMappedMultiBlockIBC(Real&           a_smallPressure,
                                     const Real&     a_gamma,
...
void GaussianMappedMultiBlockIBC::setFortranCommon(Real&           a_smallPressure,
                                         const Real&     a_gamma,
                                         const Real&     a_ambientDensity,
                                         const Real&     a_deltaDensity,
                                         const int&      a_pressure,
...
PhysMappedIBC* GaussianMappedMultiBlockIBC::new_physIBC()
{
  GaussianMappedMultiBlockIBC* retval = new GaussianMappedMultiBlockIBC();
...
void GaussianMappedMultiBlockIBC::initialize(LevelData<FArrayBox>& a_U)
{
  ...
  for (DataIterator dit = a_U.dataIterator(); dit.ok(); ++dit)
    {
      ...
      // Box of current grid
      Box uBox = UFab.box();
      // petermc, 4 Sep 2012: not for multiblock
      // uBox &= m_domain;
      ...
      FORT_GAUSSIANMAPPED2INITF(CHF_FRA(UFab),
                                CHF_CONST_FRA(XFab),
                                CHF_BOX(uBox),
...
void GaussianMappedMultiBlockIBC::primBC(FArrayBox&             a_WGdnv,
                               ...
                               const Real&            a_time)
{
  ...
  //  {
  //    MayDay::Error("GaussianMappedMultiBlockIBC::primBC only supports periodic BCs");
  //  }
}
...
void GaussianMappedMultiBlockIBC::setBdrySlopes(FArrayBox&       a_dW,
                                      const FArrayBox& a_W,
                                      const int&       a_dir,
                                      const Real&      a_time)
{
  MayDay::Error("GaussianMappedMultiBlockIBC::setBdrySlopes not defined");
}
