...
MOLMappedPolytropicPhysics::MOLMappedPolytropicPhysics(const Real& a_smallPressure)
  :
  MOLMappedPhysics()
{
...
MOLMappedPolytropicPhysics::~MOLMappedPolytropicPhysics()
{
}

// Compute the maximum wave speed
Real MOLMappedPolytropicPhysics::getMaxWaveSpeed(const FArrayBox& a_U,
                                        const Box&       a_box)
{
  CH_assert(isDefined());
...
void MOLMappedPolytropicPhysics::soundSpeed(FArrayBox& a_speed,
                                      const FArrayBox& a_U,
                                      const Box&       a_box)
{
  CH_assert(isDefined());
...
MOLPhysics* MOLMappedPolytropicPhysics::new_molPhysics() const
{
  CH_assert(m_isBCSet);

  MOLPhysics* retval = static_cast<MOLPhysics*>
    (new MOLMappedPolytropicPhysics(m_smallPressure));
...
int MOLMappedPolytropicPhysics::numConserved() const
{
  return UNUM;
}

// Names of the conserved variables
Vector<string> MOLMappedPolytropicPhysics::stateNames()
{
  CH_assert(isDefined());
...
int MOLMappedPolytropicPhysics::numFluxes() const
{
  CH_assert(isDefined());
...
int MOLMappedPolytropicPhysics::densityIndex()
{
  return WRHO;
}

void MOLMappedPolytropicPhysics::getFlux(FArrayBox&       a_flux,
...
int MOLMappedPolytropicPhysics::numPrimitives() const
{
  CH_assert(isDefined());
...
void MOLMappedPolytropicPhysics::riemann(FArrayBox&       a_WGdnv,
...
void MOLMappedPolytropicPhysics::consToPrim(FArrayBox&       a_W,
                                            const FArrayBox& a_U,
                                            const Box&       a_box)
{
  CH_assert(isDefined());
...
Interval MOLMappedPolytropicPhysics::velocityInterval() const
{
  CH_assert(isDefined());
...
Interval MOLMappedPolytropicPhysics::vectorFluxInterval() const
{
  CH_assert(isDefined());
#if CH_SPACEDIM==1
...
int MOLMappedPolytropicPhysics::pressureIndex()
{
  CH_assert(isDefined());
...
Real MOLMappedPolytropicPhysics::smallPressure()
{
  CH_assert(isDefined());
...
int MOLMappedPolytropicPhysics::bulkModulusIndex()
{
  CH_assert(isDefined());
...
#ifdef CH_USE_HDF5
void MOLMappedPolytropicPhysics::expressions(
  HDF5HeaderData& a_expressions) const
{
