...
//-----------------------------------------------------------------------
AnalyticAdvectionIBC::
AnalyticAdvectionIBC(RefCountedPtr<ScalarFunction> a_initialState):
  PhysMappedIBC(),
...
new_physIBC()
{
  AnalyticAdvectionIBC* retval =
    new AnalyticAdvectionIBC(m_initialState);
...
primBC(FArrayBox&             a_WGdnv,
       ...
       const Real&            a_time)
{
//  if (!m_domain.is(a_dir))
//    MayDay::Error("AnalyticAdvectionIBC only supports  BCs");
}
//-----------------------------------------------------------------------
