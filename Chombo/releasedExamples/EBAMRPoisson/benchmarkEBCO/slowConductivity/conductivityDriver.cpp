...
void solve(const PoissonParameters&  a_params)
{
  ...
  for (int ilev = 0; ilev < a_params.numLevels; ilev++)
    {
      ...
      //setTrigPhi(*phi[ilev], a_params.coarsestDx, a_params);
  ...
  defineSlowConductivitySolver(solver, grids, ebisl, *botSolve, a_params);
  ...
  //solver.solveNoInit(phi, rhs, a_params.maxLevel, 0, false, false);
