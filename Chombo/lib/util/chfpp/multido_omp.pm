...
#################################################
###  This is the MultiDo_OMP processer.
### Interface is
### sub MultiDoOMPProc::procMultiDoMacros(inputfile, outputfile,
###                                SpaceDim, debug)
###
### The specification for MultiDo is:
###    CHF_MULTIDO_OMP[boxvar;indexvar1;indexvar2;indexvar3{;stride}]
###        ....
###    CHD_ENDDO
...
package MultiDoOMPProc;
use vars qw(@ISA @EXPORT @EXPORT_OK %EXPORT_TAGS $VERSION);
use Exporter;
$VERSION = 1.24;
@ISA = qw(Exporter);
@EXPORT = qw(&procMultiDoOMPMacros);
@EXPORT_OK = qw(&procMultiDoOMPMacros);
@EXPORT_TAGS= ();

sub MultiDoProc::procMultiDoOMPMacros
{
    ...
    my $beginstring = "CHF_OMP_MULTIDO";
    my $beginlen = 16;
    my $endstring = "]";
    my $endlen = 1;
    my $beginoffset = 0;
    ...
    while ($obuf  =~ m/$beginstring/ig )
    {
        ...
        if( $dimarg[$#dimarg] =~ m/^[\+\-]?[0-9]+$/ ){
            $stride = pop @dimarg ;
            if( $stride == 0 ){
                die "error: CHF_MULTIDO_OMP with stride 0 is not allowed\n";
            }
        }
