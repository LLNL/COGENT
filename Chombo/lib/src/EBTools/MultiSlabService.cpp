...
/******************/
/******************/
MultiSlabService::MultiSlabService(const Vector<Box>& a_coveredRegion)
  :GeometryService()
{
  for (int ibox=0; ibox<a_coveredRegion.size(); ibox++)
    {
      m_coveredRegion |= IntVectSet(a_coveredRegion[ibox]);
    }
  PolyGeom::setVectDx(RealVect::Unit);
}
/******************/
/******************/
MultiSlabService::~MultiSlabService()
{
}
/******************/
/******************/
bool
MultiSlabService::isRegular(const Box& a_region,
                            ...
                            const Real& a_dx) const
{
  ...
  grownBox &= a_domain;
  IntVectSet grownIVS(grownBox);
  IntVectSet interIVS = m_coveredRegion & grownIVS;
  return (interIVS.isEmpty());
}
/******************/
/******************/
bool
MultiSlabService::isCovered(const Box& a_region,
                            const ProblemDomain& a_domain,
                            const RealVect& a_origin,
                            const Real& a_dx) const
{
...
GeometryService::InOut MultiSlabService::InsideOutside(const Box&           a_region,
                                                       const ProblemDomain& a_domain,
                                                       const RealVect&      a_origin,
                                                       const Real&          a_dx) const
{
...
void
MultiSlabService::fillGraph(BaseFab<int>&      a_regIrregCovered,
                            ...
                            const DataIndex&   a_di) const
{
  ...
  grownCovIVS &= ghostRegionIVS;
  IntVectSet ivsIrreg(grownCovIVS);
