...
isValid(PyObject* a_pyObject)
{
  ...
  double value = 1.0;
  PyObject* result = PyObject_CallFunction(a_pyObject, (char*)"d", value);
  if (result == NULL)
  {
    PyErr_Clear();
    return false;
...
//-----------------------------------------------------------------------
PyUnaryFunction::
PyUnaryFunction(PyObject* a_pyObject):
  m_obj(a_pyObject)
{
...
Real
PyUnaryFunction::
operator()(Real a_x) const
{
  // Call the function with our argument.
  PyObject* result = PyObject_CallFunction(m_obj, (char*)"d", static_cast<double>(a_x));
