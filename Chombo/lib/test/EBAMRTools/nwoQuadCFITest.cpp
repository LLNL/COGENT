             ...
             const RealVect& a_dxFine)
{
  ...
  LevelData<EBCellFAB> phiFine(a_gridsFine, nvar,
                               nghost*IntVect::Unit,ebcellfactFine);
  LevelData<EBCellFAB> phiCoar(gridsCoar, nvar,
                               nghost*IntVect::Unit,ebcellfactCoar);
  ...
  Real dxFine = dxCoar[0]/nref;
  NWOEBQuadCFInterp interpOp(a_gridsFine,  gridsCoar,
                             a_ebislFine,  ebislCoar,
                             domainCoar, nref, nvar, dxFine,
                             nghost*IntVect::Unit, cfivs);

  int isrc = 0; int idst = 0; int inco = 1;
  interpOp.coarseFineInterp(phiFine, phiCoar, isrc, idst, inco);
