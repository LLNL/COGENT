...
class Logger(object):
    ...
    def __init__(self,
        ...
        # If the level is already set, leave it alone:
        if self._logger.level == Logger.NOTSET:
...
class GeneralAutoLog (object):
    ...
    ENTRY_WORD = 'START'
    EXIT_WORD  = 'END  '
    ENTRY_MESSAGE = '%s {}' % ENTRY_WORD
    EXIT_MESSAGE  = '%s {}' % EXIT_WORD


    def __init__(self, logger=None, func_name=None, debug_only=False):
    ...
    def __call__(self, func):
        ...
        def wrapper(*args, **kwds):
            ...
            if not self._log_calls_set:
                if self.debug_only:
                    self._log_entry = self.logger.debug
                    self._log_exit = self.logger.debug
                else:
                    self._log_entry = self.logger.info
                    self._log_exit = self.logger.info
                self._log_calls_set = True
            if self.func_name is None:
                self.func_name = func.__name__
            self._log_entry(self.ENTRY_MESSAGE.format(self.func_name))
            result = func(*args, **kwds)
            self._log_exit(self.EXIT_MESSAGE.format(self.func_name))
            return result

        return wrapper

def getLocalAutoLog(local_logger):
    """ Returns a subclass of local_logging.GeneralAutoLog whose __init__ uses
    local_logger.  Makes it unnecessary to duplicate this class definition in
    multiple client modules.
    """
    class LocalAutoLog (GeneralAutoLog):
        def __init__(self, func_name=None, debug_only=False):
            super(LocalAutoLog, self).__init__(
                logger=local_logger,
                func_name=func_name,
