...
void Advect::evalRHS( const MultiBlockLevelGeom&  geom,
                      ...
                      const Real                  time )
{
  CH_assert(soln.ghostVect() == 4*IntVect::Unit);
                       ...
                       LevelData<FluxBox>&         a_velocity)
{
   ...
   LevelData<FluxBox> N_face(a_velocity.disjointBoxLayout(), SpaceDim*SpaceDim, IntVect::Unit);
   coords->getPointwiseN(N_face);
   ...
   // We now have N^T*v on valid cell faces plus one ghost cell layer
   // Convert face-centered values to face averages.
   fourthOrderAverage(a_velocity);
